...
export default function InterconnectionCard() {
    ...
    return (
        <div className="bg-midnight-blue/40 backdrop-blur-md rounded-xl p-4 border border-border/20 shadow-2xl">
            <h4 className="text-[10px] font-bold text-accent uppercase tracking-widest mb-4">Hydrologic Interconnection</h4>
                ...
                {basins.map(([id, info]: [string, any]) => (
                    <div key={id} className="space-y-2 border-b border-border/10 pb-4 last:border-0">
                        <div className="flex items-center justify-between">
                            <span className="text-[10px] font-bold text-core-blue">Basin ${id}</span>
                            <div className="flex gap-1">
                                {info.shared_aquifer_risk && (
                                    <span className="bg-red-500/10 text-red-400 text-[8px] px-1.5 py-0.5 rounded border border-red-500/20">AQUIFER RISK</span>
                                )}
                                {info.transfer_system_dependent && (
                                    <span className="bg-amber-500/10 text-amber-400 text-[8px] px-1.5 py-0.5 rounded border border-amber-500/20">TRANSFER DEP</span>
                                )}
                            </div>
                        </div>
